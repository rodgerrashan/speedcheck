...
const scrollToSection = (id: string) => {
  ...
  return (
    <nav
      ref={navRef}
      onClick={() => setMenuOpen(!menuOpen)} // tap anywhere toggles
      className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-50 w-11/12 max-w-2xl rounded-2xl bg-white/30 backdrop-blur-lg px-6 transition-all duration-500 overflow-hidden ${
    ...
    >
      ...
      {/* Desktop nav */}
      <div className="hidden md:flex justify-center space-x-6 mt-1">
        {sections.map((sec) => (
          <button
            key={sec.id}
      ...
      >
        <div className="overflow-hidden flex flex-col space-y-2 mt-3">
          {sections.map((sec) => (
            <button
              key={sec.id}
              onClick={(e) => {
